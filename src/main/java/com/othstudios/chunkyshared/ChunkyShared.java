package com.othstudios.chunkyshared;
...
import com.othstudios.chunkyshared.chunky.ChunkyGateway;
import com.othstudios.chunkyshared.command.ChunkySharedCommand;
import com.othstudios.chunkyshared.config.ChunkySharedConfig;
import com.othstudios.chunkyshared.network.ConfigNetworking;
import com.othstudios.chunkyshared.scheduler.PregenScheduler;

/**
 * Common entrypoint for Chunky Shared. Wires server lifecycle and player connection events to the
 * presence-gated chunk pregeneration scheduler.
 */
public class ChunkyShared implements ModInitializer
{
    /**
     * This mod's Fabric mod identifier.
     */
    public static final String MOD_ID = "chunky-shared";
    ...
    private ChunkySharedConfig _config;
    ...
    public void onInitialize()
    {
        ...
        CommandRegistrationCallback.EVENT.register((dispatcher, buildContext, selection) -> ChunkySharedCommand.register(dispatcher, () -> _config, this::onCurveConfigChanged));
    }
    ...
    private void initializeForServer(final MinecraftServer server)
    {
        if (_config == null)
        {
            _config = ChunkySharedConfig.load(FabricLoader.getInstance().getConfigDir().resolve("chunkyshared.json"));
     ...
     * @return An identifier of the form {@code chunky-shared:<path>}.
     */
