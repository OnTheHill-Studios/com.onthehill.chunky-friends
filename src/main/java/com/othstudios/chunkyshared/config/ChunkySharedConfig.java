package com.othstudios.chunkyshared.config;
 ...
 */
public final class ChunkySharedConfig
{
    private static final Logger LOGGER = LoggerFactory.getLogger(ChunkySharedConfig.class);
     ...
     *
     * @param configFilePath Path to the config file, e.g. {@code config/chunkyshared.json}.
     * @return The loaded configuration, or a default instance if the file could not be read.
     */
    public static ChunkySharedConfig load(final Path configFilePath)
    {
        final ChunkySharedConfig config = loadInternal(configFilePath);
        config._configFilePath = configFilePath;
        return config;
    }

    private static ChunkySharedConfig loadInternal(final Path configFilePath)
    {
        if (!Files.exists(configFilePath))
        {
            final ChunkySharedConfig defaults = new ChunkySharedConfig();
            defaults.save(configFilePath);
            return defaults;
        }
        try (Reader reader = Files.newBufferedReader(configFilePath))
        {
            final ChunkySharedConfig loaded = GSON.fromJson(reader, ChunkySharedConfig.class);
            return loaded != null ? loaded : new ChunkySharedConfig();
        }
        catch (final IOException exception)
        {
            LOGGER.error("Failed to read config file at {}, falling back to defaults.", configFilePath, exception);
            return new ChunkySharedConfig();
        }
    }
