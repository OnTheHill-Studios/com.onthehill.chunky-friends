 ...
 * {@code /chunkyshared config} server command, so the GUI's buttons and the command line always agree on
...
public final class ConfigNetworking
{
     ...
     * client GUI or the {@code /chunkyshared config} command. Falls back to vanilla op level
     * {@link PermissionLevel#GAMEMASTERS} when no permission mod is installed.
     */
    public static final Identifier CONFIG_PERMISSION = Identifier.fromNamespaceAndPath(ChunkyShared.MOD_ID, "config");
     ...
     *     {@code ChunkySharedConfig.getMaxRadiusChunks}), or empty if the input isn't a recognized number
     ...
     * actual {@link ChunkySharedConfig} instance to close over, so — unlike {@link #registerPayloadTypes} —
     ...
     */
    public static void registerServerReceivers(final ChunkySharedConfig config, final Runnable onCurveChanged)
    {
     ...
     * {@code ChunkySharedCommand} call this rather than duplicating validation.
     *
     ...
     */
    public static boolean applyUpdate(final ChunkySharedConfig config, final int ringCount, final int maxRadiusChunks, final boolean quadratic, final Runnable onCurveChanged)
    {
    ...
    private static void handleRequest(final ServerPlayer player, final ChunkySharedConfig config)
    {
        ...
        if (!hasPermission((PermissionContextOwner) player))
        {
            ...
            player.sendSystemMessage(Component.translatable("message.chunky-shared.config.no_permission"));
            return;
        }
        ServerPlayNetworking.send(player, toStatePayload(config));
    }

    private static void handleUpdate(final ServerPlayer player, final ConfigUpdatePayload payload, final ChunkySharedConfig config, final Runnable onCurveChanged)
    {
        ...
        if (!applyUpdate(config, payload.ringCount(), payload.maxRadiusChunks(), payload.quadratic(), onCurveChanged))
        {
            ...
            player.sendSystemMessage(Component.translatable("message.chunky-shared.config.invalid_values"));
            return;
        }
        LOGGER.info("Config updated by {} ({}): ringCount={}, maxRadiusChunks={}, quadratic={}.",
                player.getGameProfile().name(), player.getUUID(), payload.ringCount(), payload.maxRadiusChunks(), payload.quadratic());
        player.sendSystemMessage(Component.translatable("message.chunky-shared.config.saved"));
        ServerPlayNetworking.send(player, toStatePayload(config));
    }

    /**
     * Sends the given player a current-values snapshot followed by an instruction to open the configuration
     * screen. Called from {@code /chunkyshared gui}, executed by a real connected player.
     *
     ...
     */
    public static boolean openGuiFor(final ServerPlayer player, final ChunkySharedConfig config)
    {
     ...
     */
    public static ConfigStatePayload toStatePayload(final ChunkySharedConfig config)
    {
